...
def show_object_with_hand(args):
    # load data
    ...
    for result_name in result_names:
        ...
        # object_mesh = tm.load(os.path.join('data/DFCData/meshes', object_code, 'coacd/decomposed.obj')).apply_scale(
        #     object_scale)
        ...
        # hand mesh
        hand_translation = result['sampled_rotation'][args.num].numpy().T @ result['translation'][
            args.num].numpy() if args.canonical_frame else result['translation'][args.num].numpy()
        ...
        tta_hand_translation = result['sampled_rotation'][args.num].numpy().T @ result['tta_hand_pose'][args.num][
                                                                                :3].numpy() if args.canonical_frame else \
        result['tta_hand_pose'][args.num][:3].numpy()
        tta_hand_rotation = np.eye(3) if args.canonical_frame else pytorch3d.transforms.axis_angle_to_matrix(
            result['tta_hand_pose'][args.num][3:6]).numpy()
...
def show_object_with_multi_sample_hand(args):
    builder = ShadowHandBuilder()
    # object_name = "sem_Bottle-e101cc44ead036294bc79c881a0e818b"
    # object_name = "sem_Bottle-3702fc385b6c7708fc33503fd88ecb34"
    object_name = "sem_Bottle-908e85e13c6fbde0a1ca08763d503f0e"

    result_root = "/home/pxn-lyj/Egolee/programs/UniDexGrasp_liyj/dexgrasp_generation/local_files/data/infer_result"
    # result_root = "/home/pxn-lyj/Egolee/programs/UniDexGrasp_liyj/dexgrasp_generation/local_files/data/infer_result_seed233"
    # result_root = "/home/pxn-lyj/Egolee/programs/UniDexGrasp_liyj/dexgrasp_generation/local_files/data/infer_result_seed1024"
    # result_root = "/home/pxn-lyj/Egolee/programs/UniDexGrasp_liyj/dexgrasp_generation/local_files/data/infer_result_seed2048"
    result_names = [name for name in os.listdir(result_root) if name.startswith(object_name)]

    print(result_names)
    object_pc_plotly = None
    tta_hand_mesh_plotlys = []
    ...
    for result_name in result_names:
        ...
        tta_hand_mesh_plotlys.append(tta_hand_mesh_plotly)

    tta_hand_mesh_plotlys.append(object_pc_plotly)
    # visualize
    # fig = go.Figure([object_pc_plotly, hand_mesh_plotly, tta_hand_mesh_plotly])
    # fig = go.Figure([object_pc_plotly, hand_mesh_plotly])
    # fig = go.Figure([object_pc_plotly, tta_hand_mesh_plotly])
    fig = go.Figure(tta_hand_mesh_plotlys)
    fig.update_layout(scene_aspectmode='data')
    fig.show()
...
if __name__ == '__main__':
    ...
    # 显示单次推理的结果
    show_object_with_hand(args)

    # 对单个物体进行多次采样的结果
    # show_object_with_multi_sample_hand(args)
